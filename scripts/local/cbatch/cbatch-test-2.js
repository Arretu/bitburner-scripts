...
export async function main(ns) {
	...
	await cbatch.tPrep(ns,target,pHosts[0])
	...
	while (true) {
		...
		if (cTarget.moneyMax - cTarget.moneyAvailable > 0 || cTarget.hackDifficulty - cTarget.minDifficulty > 0) {
			logNum++
			ns.tprint("Failed status check after block "+blockID)
			ns.write("/debug/cbatch-test-2-debug.txt", "------------" + logNum + "------------" + "\n", "a")
			ns.write("/debug/cbatch-test-2-debug.txt", "Batch on block " + blockID.toString() + " finished." + "\n", "a");
		...
		if (repFired == false) {
			if (cbatch.launchCheckBatch(ns, goalPerc, ramPool.get(blockID), target, delay, 0, blockID, port) == true) {
				repFired = true;
				repCount = 0;
			}
		}
			...
			repCount++
			if (repCount >= desiredBatches) {
				repFired = false;
			}
			cbatch.launchBatch(ns, goalPerc, ramPool.get(blockID), target, delay, 0, blockID, port)
		}

		
		ns.print(blockID," relaunched.")
